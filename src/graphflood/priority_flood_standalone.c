...
/*
See topotoolbox.h for details
Variant of priority flood + epsilon that also computes a multiple flow compatible topological ordering (Stack) on the go
Note: this version is a bit slower as it needs to push all nodes in the priority queue including the ones in pit (no fifo queue)
By experience the slowing down factor is small for most cases ( low amount of depressions, ~10% max slow down)
*/
TOPOTOOLBOX_API
void compute_priority_flood_plus_topological_ordering(float* topo, GF_UINT* stack, uint8_t* BCs, GF_UINT* dim, bool D8) {
	...
	while(pfpq_empty(&open) == false){
		...
		GF_FLOAT current_elev = topo[node];
