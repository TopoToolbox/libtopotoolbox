...
It creates a generic framework to iterate in D4 or D8 generically and future-proofed.

Typically, these are called at the top of a function (cheap to generate if done once per function)
...
/*
@Brief Helper function to create the offset arrays for 2D neighbouring in D4 (cardinal only)
To be called with int8_t off[N_neighbour(D8)] to feed them;
*/
void generate_offset_D4(int8_t* off0, int8_t* off1);

/*
@Brief Helper function to create the offset arrays for 2D neighbouring in D8 (cardinal + diagonals)
To be called with int8_t off[N_neighbour(D8)] to feed them;
*/
void generate_offset_D8(int8_t* off0, int8_t* off1);

/*
@Brief Helper function to create the offset arrays for 1D neighbouring in D4 (cardinal only)
1D neighbouring uses flat indices: ID = row * ncolumns + column; for ROW MAJOR or ID = column * nrows + row; for COLUMN MAJOR
...
/*
@Brief Helper function to create the offset arrays for 1D neighbouring in D8 (cardinal only)
1D neighbouring uses flat indices: ID = row * ncolumns + column; for ROW MAJOR or ID = column * nrows + row; for COLUMN MAJOR
...
/*
@Brief Takes the indices ofdim[0] and dim[1] [row,col (row major) or col row (column major)] and returns the flat index
